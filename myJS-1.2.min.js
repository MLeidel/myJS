...
return d.getDay();},todayMon:function(objdate){let d;if(objdate!==undefined){d=objdate;}else{d=new Date();}
...
return false;else
str=str.toString();return str.replace(/\w\S*/g,function(txt){return txt.charAt(0).toUpperCase()+txt.substr(1).toLowerCase();});},numfix:function(n,p){if(p===undefined){return parseFloat(n.toFixed(2));}else{return parseFloat(n.toFixed(p));}},setCookie:function(cname,cvalue,exdays){let d=new Date();d.setTime(d.getTime()+(exdays*24*60*60*1000));let expires="expires="+d.toUTCString();if(exdays)
